...
/* Driver memory technology like DDR, LPDDR, NAND Flash, eMMC and HBM for
 * JEDEC standard devices.
 * https://aticleworld.com/what-is-jedec-and-how-it-works/#google_vignette
 *
 * This driver only works for SPI based JEDEC devices.
 * https://aticleworld.com/spi-communication-protocol/
 *
 * This implementation is based from these sources:
 * 1. https://aticleworld.com/jedec-memory-interface-in-cpp/
 * 2. this mtd module : w25*.c
 * 3. ardupilot libraries : AP_Logger_W25NXX.cpp, AP_Logger_Flash_JEDEC.cpp
 */
...
/* JEDEC parts can be driven with either SPI mode 0
 * (CPOL=0 and CPHA=0) or mode 3 (CPOL=1 and CPHA=1).
 * But I have heard that other devices can operate in mode 0 or 1.
 * So you may need to specify CONFIG_JEDEC_SPIMODE to select the best mode
 * for your device.  If CONFIG_JEDEC_SPIMODE is not defined, mode 0 will be
 * used.
 */
...
/* SPI Frequency.  May be up to 133MHz. */

#ifndef CONFIG_JEDEC_SPIFREQUENCY
#  define CONFIG_JEDEC_SPIFREQUENCY 20000000 // 20MHz
#endif

/****************************************************************************
 * JEDEC Flash Commands
 * https://aticleworld.com/jedec-flash-commands-what-they-are-why-they-matter-in-embedded-systems/
 ****************************************************************************/

/* read Operations*/
#define JEDEC_READ_PAGE_DATA       0x13 // Read page data (W25N01GV)
#define JEDEC_READ_DATA            0x03 // Read data bytes (SPI line)
#define JEDEC_FAST_READ            0x0B // Fast read (SPI line + dummy cycles)
#define JEDEC_4B_FAST_READ         0x0C // 4-byte address fast read (SPI line + dummy cycles, W25N01GV)
#define JEDEC_DUAL_OUTPUT_READ     0x3B // Dual read (DQ0, DQ1 lines)
#define JEDEC_4B_DUAL_OUTPUT_READ  0x3C // 4-byte address dual read (W25N01GV)
#define JEDEC_DUAL_IO_READ         0xBB // Address + data (DQ0, DQ1 lines)
#define JEDEC_4B_DUAL_IO_READ      0xBC // 4-byte address dual read (W25N01GV)
#define JEDEC_QUAD_READ            0x6B // Quad read (DQ0-DQ3 lines)
#define JEDEC_4B_QUAD_OUTPUT_READ  0x6C // 4-byte address quad read (W25N01GV)
#define JEDEC_QUAD_IO_READ         0xEB // Address + data (DQ0-DQ3 lines)
#define JEDEC_4B_QUAD_IO_READ      0xEC // 4-byte address quad read (W25N01GV)
#define JEDEC_READ_UNIQUE_ID       0x4B // Read Unique ID (8 bytes)
#define JEDEC_READ_MANUFACTURE_ID  0x90 // Read Manufacturer/Device ID
#define JEDEC_DEVICE_ID            0x9F // JEDEC ID read
#define JEDEC_READ_DATA            0x03 // Read data bytes (SPI line)

/* Program (Write) Operations */
#define JEDEC_PAGE_WRITE           0x02 // Program up to a full page
#define JEDEC_QUAD_PAGE_WRITE      0x32 // Quad page program
#define JEDEC_RANDOM_WRITE         0x84 // Random byte program (W25N01GV)
#define JEDEC_QUAD_RANDOM_READ     0x34 // Random quad byte program (W25N01GV)
#define JEDEC_PROGRAM_EXECUTE      0x10 // Execute program operation (W25N01GV)

/* Erase Operations */
#define JEDEC_SECTOR4_ERASE        0x20 // 4k erase, sector erase
#define JEDEC_BLOCK32_ERASE        0x52 // 32K erase, block 32 KB erase
#define JEDEC_BLOCK64_ERASE        0xD8 // 64K erase, block 64 KB erase
#define JEDEC_BULK_ERASE           0xC7 // 0x60 also works, chip erase
#define JEDEC_CHIP_ERASE           0x60 // Chip erase, also 0xC7 works

/* Register & Status Commands */
#define JEDEC_READ_STATUS          0x05 // Read status register-1, 0x0F also works (W25N01GV)
#define JEDEC_READ_STATUS2         0x35 // Read status register-2
#define JEDEC_READ_STATUS3         0x15 // Read status register-3
#define JEDEC_WRITE_STATUS         0x01 // Write to status registers, 0x1F also works (W25N01GV)
#define JEDEC_WRITE_ENABLE         0x06 // enable writing to the flash memory
#define JEDEC_WRITE_DISABLE        0x04 // disable write access to the memory

/* Power & Protection */
#define JEDEC_DEVICE_RESET         0xFF // Reset the device (W25N01GV)
// #define JEDEC_DEVICE_RESET         0x99 // Reset the device
#define JEDEC_DEEP_POWER_DOWN      0xB9 // Deep power down
#define JEDEC_RELEASE_POWER_DOWN   0xAB // Release from deep power down
#define JEDEC_ENABLE_RESET         0x66 // Enable the reset command
#define JEDEC_UNLOCK_GLOBAL_BLOCK  0x98 // Unlock all block protection (some device)

/* Advanced / Optional Commands */
#define JEDEC_TOGGLE_BIT	   0xA0 // Toggle during self-test (some device)
#define JEDEC_SET_BURST_LENGTH	   0xC0 // Set burst mode length (some device)
#define JEDEC_READ_SFDP	   	   0x5A // Read Serail Flash Discoverable Parameters
#define JEDEC_ENTER_4B_ADDR_MODE   0xB7 // Enter 4-byte address mode
#define JEDEC_EXIT_4B_ADDR_MODE    0xE9 // Exit 4-byte address mode
#define JEDEC_BANK_REG_ACCESS      0x16 // for bank access memory larger than 128MB

/* NAND Bad Block Management*/
#define JEDEC_SWAP_BLOCK       	   0xA1 // Swap block
#define JEDEC_READ_BBM_LUT     	   0xA5 // Read bad block information
#define JEDEC_LAST_ECC_FAIL_ADDR   0xA9 // Read last ECC failure page address

/* JEDEC Status Register Bits (from ardupilot) */
...
#define JEDEC_ID_WINBOND_W25N01	       0xEFAA21
